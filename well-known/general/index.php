...
.bg {
    background-image: url('../general/assets/dist/img/prepayment.jpg');
   ...
   <script>
        $(document).ready(function() {
            ...
            $("#search").click(function() {
                ...
                var $input = $('<button type="submit" name="paynow" class="w-30 btn btn-lg btn-danger mt-3" >Pay Now</button>');
                var pol_no = $("#tpolicy").val();
                // var $link = 'http://116.12.80.89:8001/ThirdPartyPolicy/GetPolicy?policyNo=' + pol_no;
                var $link = 'https://online.ci.lk/pol_info.php?policyNo=' + pol_no + '&type=gen';
                ...
                $.getJSON($link, function(result) {
                    ...
                    if (result.policyNo != null) {
                        var period = result.fromDate + " to " + result.toDate;
                        $('.policy').empty().val(result.policyNo);
                        $('.vehi').empty().val(result.vehicleNo);
                        $('.name').empty().val(result.customerName);
                        $('.period').empty().val(period);
                        $('.pre').empty().val(result.premiumOutstanding);


                        if (result.premiumOutstanding > 0) {
                            $('.pay').empty().val(result.premiumOutstanding);
...
<body class="bg shadow-lg">
    <div class="container py-4">
                                ...
                                <div class="row g-4">
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            ...
                                            <!-- PYB209000020 -->
                                            <label for="policyNumber" class="required-field">Policy No</label>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            <input type="text" class="form-control pay" id="txtPaymentAmount" name="txtPaymentAmount" disabled placeholder="Enter payment amount" pattern='^[1-9]\d{0,2}(?:,?\d{3})*(?:\.\d{2})?$' required>
        ...
        <!-- //formValidation js -->
        <!-- <script type="text/javascript" src="assets/dist/js/genscript.js"></script> -->
         <script>
